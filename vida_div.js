    ...
    var vida = function(element, options)
    {
        ...
        settings.verovioWorker.onmessage = function(event){
            switch (event.data[0]){
                ...
                case "loadedPage":
                    settings.currentPage = event.data[1];
                    settings.svg = event.data[2];
                    settings.totalPages = event.data[3];
        ...
        function refreshVerovio(newData)
        {
            ...
            else
            {
                ...
                settings.verovioWorker.postMessage(['redoLayout', referenceID]);
                // can I take care of this someplace else?
                $(".vida-loading-popup").remove();
            }
//            checkNavIcons();
//            reloadMEI();
...
*/
        $(".vida-next-page").on('click', function()
        {
            if (settings.currentPage < settings.totalPages)
            {
                ...
                settings.verovioWorker.postMessage(['loadPage', settings.currentPage]);
                checkNavIcons();
            }
        });
